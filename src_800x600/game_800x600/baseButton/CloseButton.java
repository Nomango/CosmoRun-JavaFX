...
public class CloseButton extends Group {
	...
	private Rectangle r1 = new Rectangle(-2, -16, 4, 32);
	private Rectangle r2 = new Rectangle(-2, -16, 4, 32);
	...
	private void initCoordinate() {
		// 外层六边形坐标
		for (int i = 0; i < 7; i++) {
			p.getPoints().add(36.85 * Math.cos(Math.toRadians(i * 60)));
			p.getPoints().add(36.85 * Math.sin(Math.toRadians(i * 60)));
			clickedArea.getPoints().add(36.85 * Math.cos(Math.toRadians(i * 60)));
			clickedArea.getPoints().add(36.85 * Math.sin(Math.toRadians(i * 60)));
		}

		// 内层六边形坐标
		for (int i = 0; i < 7; i++) {
			p.getPoints().add(32.16 * Math.cos(Math.toRadians(-i * 60)));
			p.getPoints().add(32.16 * Math.sin(Math.toRadians(-i * 60)));
		}
	}
