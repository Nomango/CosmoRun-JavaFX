...
import game_800x600.object.Ball;
import game_800x600.object.Floor;
import game_800x600.object.FloorManager;
import game_800x600.object.Floors;
...
public class Display {
	...
	private static void initText() {
		scoreText.setLayoutX(Game.width / 2 - 15);
		scoreText.setLayoutY(70);
		scoreText.setFill(Color.WHITE);
		scoreText.setFont(Font.font("Gill Sans MT Condensed",FontWeight.BOLD, 60));
		...
		processTimeline = new Timeline(new KeyFrame(Duration.millis(15), e -> {
			...
			if (!ball.isOn()) {
				...
				if (Floors.isOutOfFloors()) {
					...
					// 游戏加速
					speed = (float)((-1.667e-6 * Game.score + 0.0013) * Game.score + 0.6) * 0.667f;
				}
			}
		}));
